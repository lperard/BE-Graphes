...
public class BinaryHeap<E extends Comparable<E>> implements PriorityQueue<E> {
    ...
    public void remove(E x) throws ElementNotFoundException {
    	...
    	if (index_x >= this.currentSize) {
    		throw new ElementNotFoundException(x);
    	}
    	else {
    		E fin = this.array.get(this.currentSize - 1);
    		this.array.set(index_x, fin);
    		this.currentSize--;
    			if (index_x < this.currentSize) {
    ...
    /*if (this.currentSize == 0) {//array vide
	throw new ElementNotFoundException(x);
}
if (this.currentSize == 1) {
	...
	//si un seul elem et c'est celui la
	else {
		this.currentSize--;
	}
}
else { //on doit parcourir le tas pour trouver notre element et l'enlever
	E memoire = null;
	int index_x = -1;
	for (int i = 0; i < this.currentSize ; i++) { //on cherche la liste pour trouver l'element et le changer avec le dernier
		E element = this.array.get(i);
		if (element == x) {
			element = this.array.get(this.currentSize-1);
			memoire = element;
			index_x = i;
		}
